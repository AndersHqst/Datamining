...
def numeric_attribute_histogram(plt, frame, title, num=10, interquantile=False, mmm=True, binary=False):
    ...
    missing_values = len(frame) - len(clean_frame) # Added as new line under title
    ...
    if not binary:
        ...
        plt.ylabel('sites')
        plt.hist([[0] * sum(vals == 0), [1] * sum(vals == 1)], bins=bins, histtype='bar', color=['r', 'b'], label=['No', 'Yes'])
        plt.legend()
